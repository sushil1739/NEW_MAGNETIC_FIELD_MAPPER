...
def _slice_for_plane(
    ...
    if plane == "xy":
        fixed_col, dims, labels = 2, (0, 1), ("X [mm]", "Y [mm]")
    elif plane == "xz":
        fixed_col, dims, labels = 1, (0, 2), ("X [mm]", "Z [mm]")
    elif plane == "yz":
        fixed_col, dims, labels = 0, (1, 2), ("Y [mm]", "Z [mm]")
    else:
...
def plot_3d(args, data: np.ndarray):
    ...
    ax.set_ylabel("Y [mm]")
    ax.set_zlabel("Z [mm]")
    ax.set_title(f"TMS 3D magnetic field — {args.component.upper()}")
    fig.tight_layout()
    return fig
